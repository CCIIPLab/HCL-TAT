...
class Proto(nn.Module):
    ...
    def forward(self, support_set, query_set, N, K, Q):
        # encode
        support_emb = self.encoder(support_set['tokens'], support_set["att-mask"])   # B*N*K, max_len, feature_size
        query_emb = self.encoder(query_set['tokens'], query_set['att-mask'])       # B*N*K, max_len, feature_size
